...
for iii in range(0, len(df['task_image_name'])):
    ...
    if os.path.exists(
            root_folder_path + name_list[iii].replace('/0/', '/' + str(dialog_phase) + '/') + "_" + str(
                dialog_phase) + ".pickle") \
            and os.path.exists(
        root_folder_path + name_list[iii].replace('/0/',
                                                  '/' + str(dialog_phase) + '/image_sample_') + "_" + str(
            dialog_phase) + ".jpg"):
        print()
        # /Users/fanyue/xview/full_dataset/1855/0/1855.tif.pickle
        print(root_folder_path + '/'.join(name_list[iii].split('/')[:-1]) + '/' + name_list[iii].split('/')[
            1] + ".tif.pickle")
        p_dic = pickle.load(
            open(root_folder_path + '/'.join(name_list[iii].split('/')[:-1]) + '/' + name_list[iii].split('/')[
                1] + ".tif.pickle",
                 "rb"))
        ...
        # /Users/fanyue/xview/full_dataset/1855/2/3_2.pickle
        print(root_folder_path + name_list[iii].replace('/0/', '/' + str(dialog_phase) + '/') + '_' + str(
            dialog_phase) + ".pickle")
        ...
        length_of_traj = [1] + length_of_traj
        ...
        if 'rej' in dialog or 'rej' in question or len(angle_list) == 0:
            ...
            #             pickle.dump(p_dic, open(root_folder_path + name_list[iii].replace('full_dataset', 'full_dataset_filtered').replace('/0/',
            #                                                                        '/' + str(dialog_phase) + '/') + '_' + str(
            #                 dialog_phase) + ".pickle",
            #                              "wb"))
            continue

        # /Users/fanyue/xview/full_dataset/1855/0/0.pickle
        ...
        path = root_folder_path + name_list[iii].replace('/0/',
                                                         '/' + str(dialog_phase) + '/image_sample_') + "_" + str(
            dialog_phase) + ".jpg"

        im_full_map = cv2.imread(root_folder_path + 'train_images/' + name_list[iii].split('/')[1] + ".tif", 1)
        im_resized_copy = cv2.resize(im_full_map,
        ...
        ############## optimize the traj ####################
        # p_dic = pickle.load(open(root_folder_path + name_list[iii].replace('/0/',
        #                                                                    '/' + str(dialog_phase) + '/') + '_' + str(
        #     dialog_phase) + ".pickle",
        #                          "rb"))
        # pos_list = p_dic['pos_list']
        assert type(pos_list[0]) == type(np.array([])), iii
        print(length_of_traj)

        new_pos_list_of_all_phase = []
        for i in range(len(length_of_traj) - 1):

            pos_list_of_one_phase = pos_list[length_of_traj[i] - 1:length_of_traj[i + 1]]
            new_pos_list = [pos_list_of_one_phase[0]]
            while (1):
                for p_i in range(len(pos_list_of_one_phase[1:][::-1])):
                    p = pos_list_of_one_phase[1:][::-1][p_i]

                    if np.linalg.norm(img_to_gps_coords(np.mean(new_pos_list[-1], axis=0)) - img_to_gps_coords(
                            np.mean(p, axis=0))) < \
                            np.linalg.norm(img_to_gps_coords(np.mean(new_pos_list[-1], axis=0)) - img_to_gps_coords(
                                new_pos_list[-1][0])) * 0.4:
                        new_pos_list.append(p)
                        print(p_i)
                        break
                if np.linalg.norm(
                        img_to_gps_coords(np.mean(new_pos_list[-1], axis=0)) - img_to_gps_coords(
                            np.mean(pos_list_of_one_phase[-1], axis=0))) < 0.00001:
                    break
            new_pos_list_of_all_phase.append(new_pos_list)
        new_length_of_traj = []
        for i in range(len(new_pos_list_of_all_phase)):
            new_length_of_traj.append(len(new_pos_list_of_all_phase[i]))
        print(new_length_of_traj)
        ...
        pos_list = []
        for i in range(len(new_pos_list_of_all_phase)):
            pos_list += new_pos_list_of_all_phase[i]
        ...
        #         plt.imshow(im_resized_copy[int(im_min_boundary[1]):int(im_max_boundary[1]),
        #                     int(im_min_boundary[0]):int(im_max_boundary[0]), ::-1])

        #         im_full_map = cv2.imread(path, 1)
        #         cv2.imshow('viewer', im_full_map)
        os.system('mkdir -p ' + '/'.join(path.replace('full_dataset', 'full_dataset_filtered').split('/')[:-1]))
        cv2.imwrite(path.replace('full_dataset', 'full_dataset_filtered'),
                    im_resized_copy[int(im_min_boundary[1]):int(im_max_boundary[1]),
                    int(im_min_boundary[0]):int(im_max_boundary[0])])
